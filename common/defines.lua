...
NDiplomacy = {
	...
	SUZERAIN_DEFENDER_CALL_DECLINE_COST = 200,	-- Prestige cost for a suzerain refusing to join a defensive war, when they are supposed to alway accept
	...
	MINIMUM_BUY_FAVOR_COST = 25,
	BUY_FAVOR_INCOME_SCALEFACTOR = 25,
	...
	DEMESNE_MAX_SIZE_STEWARDSHIP_MULT = 0.11,	-- Extra Max Demesne Size from ruler and spouse intrigue
	...
	CONCLAVE_VASSAL_LIMIT_KING_MULT = 20.0, 				-- Extra Vassal Limit from the ruler's rank
	CONCLAVE_VASSAL_LIMIT_EMPEROR_MULT = 30.0, 			-- Extra Vassal Limit from the ruler's rank
	...
	GAME_RULE_PROVINCIAL_REVOLT_RARE_DIVIDER = 2.5,				-- Divider to decrease risk of revolt when using the "provincial revolt: rare" game rule
	...
	OFFER_VASSALIZATION_INTERACTION_ENABLED = 0,						-- Should this action be used at all?
	...
	ASK_TO_EMBARGO_INTERACTION_ENABLED = 0,						-- Should this action be used at all?		
	...
	MAKE_CONSORT_INTERACTION_MONEY = 50000,
	...
	MAKE_CONSORT_INTERACTION_ENABLED = 0,						-- Should this action be used at all?		
	...
	START_COALITION_INTERACTION_ENABLED = 0,						-- Should this action be used at all?	
	...
	LEAVE_COALITION_INTERACTION_ENABLED = 0,						-- Should this action be used at all?	
	...
	STOP_ENFORCE_PEACE_INTERACTION_ENABLED = 0,				-- Should this action be used at all?	
	...
	ASK_REALM_PEACE_INTERACTION_ENABLED = 0,				-- Should this action be used at all?	
	...
	OFFER_CONSORT_INTERACTION_ENABLED = 0,						-- Should this action be used at all?	
	...
	MAX_INFAMY_GAIN = 0,										-- This is the max infamy gained from a single source
...
NCouncil = {
	...
	REGENCY_VOTING = "laws declare_war_interaction revoke_title_interaction imprison_character_interaction grant_landed_title_interaction exile_imprisoned_interaction execute_imprisoned_interaction release_from_prison_interaction ransom_character_interaction retract_vassal_interaction transfer_vassal_interaction ask_for_vassalization_interaction release_vassal_interaction absorb_clan_interaction grant_lp grant_lp_landless revoke_royal_privilege grant_royal_privilege seek_raiding_redress declare_feud insult_character break_non_aggression_pact NW_grant_command vassal_castrate_decision_AGOT vassal_blinding_decision_AGOT banish_prisoner take_prisoner_as_wife torture_prisoner prisoner_to_oubilette prisoner_torture prisoner_mutilate prisoner_humiliation sell_prisoner enslave_prisoner recruit_prisoner ransom_all make_noble nominate_prince revoke_freehold execute_decision demand_return_of_valyrian_steel form_alliance_interaction dissolve_alliance_interaction form_non_aggression_pact_interaction ask_to_embargo",					-- In regencies the council votes on these issues
...
NInfamy = {
	REALM_SIZE_BASE_MODIFIER = 0,
	REALM_SIZE_GROWTH_MODIFIER = 0.0,							-- The bigger you are, the more dangerous you are, modifier for when nation is growing
	REALM_SIZE_SHRINK_MODIFIER = 0.0,							-- The bigger you are, the more dangerous you are, modifier for when nation is shrinking
	WAR_REALM_CHANGE_VALUE = 0.0, 								-- How much base value for realm growth/shrink is worth in a war.
	INDEPENDENCE_REALM_CHANGE_VALUE = 0.0,	 					-- How much base value for realm growth/shrink is worth when giving a vassal independence.
	INHERITANCE_CHANGE_VALUE = 0.0,	 						-- How much base value for realm growth/shrink is worth when inheriting titles.
	VASSAL_CHANGE_VALUE = 0.0,	 								-- How much base value for realm growth/shrink is worth when inheriting titles.
	INFAMY_DECAY_BASE = 100,	
	MIN_INFAMY_DECAY = 0.2,									-- The minimum infamy decay regardless of troop strength.
	MAX_INFAMY_DECAY = 0.99,										-- The maximum infamy decay regardless of troop strength.
	MILITARY_STRENGTH_DECAY_BASE = 1,							-- Divider on military strength value that is used for the logarithmic part of the decay value.
	MAX_INFAMY_PER_WAR_PROVINCE = 0,							-- A single province taken in war can give at most this much threat
	MIN_INFAMY_PER_WAR_PROVINCE = 0,							-- A single province taken in war can give at most this much threat
},
...
NCharacter = {
	...
	LUNATIC_DIVINE_BLOOD_TRAIT_CHANCE_FACTOR = 0.25,	-- Inbreeding: Multiplier to the base chance
	...
	NON_AGGRESSION_PACT_BLOCKS_FACTIONS = 0,		-- DEFUNCT! MOVED TO GAME RULES -- If set to 1 then factions will be blocked for vassals that have a non-aggression pact with their liege.
	...
	LATE_PORTRAIT_AND_UNIT_AFTER_YEAR = 9990,		-- After this date, late versions of Units and Portraits will be used if available
	...
	INHERITED_OPINION_FACTOR = 0.25,					-- This is the factor of the opinion of the previous holder that gets inherited
	...
	TREASURY_CHANCE_TO_DISAPPEAR_STANDARD = 0,	-- Chances that artifacts disappear on inheritance
	TREASURY_CHANCE_TO_DISAPPEAR_NO_HEIR = 0,		-- Chances that artifacts disappear on inheritance when the dying character doesn't have an heir
},
...
-- Dynasty Prestige is the sum of current and previous holdings of all living and dead members of a dynasty
	BARON_TITLE_DYNASTY_PRESTIGE = 5,
	COUNT_TITLE_DYNASTY_PRESTIGE = 15,
	DUKE_TITLE_DYNASTY_PRESTIGE = 20,
...
-- Title Creation Cost	
	...
	DUKE_CREATION_COST = 50, -- Testing Titular Titles! Was 200.
	KING_CREATION_COST = 100, -- Testing Titular Titles! Was 400.
	EMPEROR_CREATION_COST = 500, -- Testing Titular Titles! Was 800.
...
-- Title Revokation Prestige Cost
	...
	EMPEROR_REVOKE_PRESTIGE_COST = 400000, -- Unused, impossible
...
-- Title Grant Decadence Reduction (Multiplied with current decadence)
	...
	MILITARY_GOV_CREATE_KINGDOMS = 1,		-- If Mercenaries and holy orders are allowed to create kingdom titles
...
-- Title creation and usurpation requirements	
	...
	EMPIRE_DEJURE_COUNTY_LIMIT_TO_CREATE = 0.51,		-- Fraction of de jure counties that you must control to create an imperial title
	EMPIRE_DEJURE_COUNTY_LIMIT_TO_USURP = 0.51,		-- Fraction of de jure counties that you must control to usurp an imperial title	
	
	CUSTOM_TITLE_COLOR_OFFSET = 0.35					-- The offset from the base title color when creating a new title
},

NReligion = {
	CREATE_ANTIPOPE_PRESTIGE_COST = 100000,
	EXCOM_MIN_AUTHORITY = 0.7,
	...
	AUTHORITY_FROM_HOLY_SITE = 0.05,					-- Authority from each holy site held
	...
	ELECTOR_TITLE_OPINION_FACTOR = 2.5,
	ELECTOR_TITLE_REALM_POWER_FACTOR = 0.5,
	ELECTOR_TITLE_CULTURE_GROUP_FACTOR = 50,
	ELECTOR_TITLE_CULTURE_FACTOR = 50,
	ELECTOR_TITLE_LEARNING_FACTOR = 1,
	ELECTOR_TITLE_PIETY_FACTOR = 0.2,
	ELECTOR_TITLE_AGE_FACTOR = 1.5,
	ELECTOR_TITLE_AT_CAPITAL_FACTOR = 100,
	ELECTOR_FAMOUS_DYNASTY_FACTOR = 0.025,
	ELECTOR_TRAITS_FACTOR = 1.5,					-- Effect of personality traits that the church likes
...
NEconomy = {
	...
	TRADE_POST_COST_INC_DIST = 0.001,				-- The % increase in cost due to distance to closest connected province
	TRADE_POST_OPINION_EFFECT = 0.3,				-- The bonus or penalty depending on topliege opinion
...
NNomad = {
	...
	POPULATION_TAX_MULTIPLIER = 0.00075,			-- Monthly tax income determined by the current population
	MAX_POPULATION_EMPTY_HOLDING_MULTIPLIER = 1800, -- Max population increase for each empty holding in provinces where the capital is also an empty holding
	MAX_MANPOWER_POPULATION_MULTIPLIER = 0.4,		-- How big max manpower will be, determined by the current population value
	...
	START_POPULATION = 0.6,						-- Starting population, multiplied with max population
	START_MANPOWER = 0.7,							-- Starting manpower, multiplied with max manpower
	...
	MAX_CLANS = 20,									-- The maximum amount of clans, regardless of realm size
	...
	GARRISON_UNUSED_MANPOWER_MULTIPLIER = 0.15,		-- How much of the unused manpower that is used for the garrison
	GARRISON_MAX_UNUSED_MANPOWER_MODIFIER = 1500,	-- The max value for how much unused manpower can modifiy the garrison
	
	MAX_HOLDINGS_IN_NOMADIC_PROVINCE = 2,			-- Counties with this number of holdings or less are counted as a nomadic province that clans can demand

	ALWAYS_GENERATE_NOMADS = 1,						-- If turned on will generate nomads even if they are not playable
...
NMilitary = {
	...
	NUMBER_OF_RETREAT_DAYS = 6,						-- Number of days before you retreat successfully
	...
	HOLDING_LEVY_SIZE_OWNER_MARTIAL_BASE = 1,	
	HOLDING_LEVY_SIZE_OWNER_MARTIAL_MULT = 0.0,	-- Levy size modifier: Each point of the holding owner's Martial skill is multiplied with this and added to HOLDING_LEVY_SIZE_OWNER_MARTIAL_BASE
	MERCENARY_MAINTENANCE_FACTOR = 0.5,				-- Maintenance factor for NORMAL mercenary units
	MERCENARY_VASSAL_MAINTENANCE_FACTOR = 0.25,		-- Maintenance factor for VASSALIZED mercenary units
	MERCENARY_HIRE_COST_FACTOR = 0.15,				-- Hire cost factor for mercenary units
	...
	VASSAL_MERC_HIRE_COST_FACTOR = 0.4,			-- Fraction of nominal hire cost when hiring vassal mercenaries OR Holy Orders
	...
	BATTLE_WARSCORE_WORTH = 135,					-- Warscore from battles are multiplied with this value
	...
	BATTLE_WARSCORE_HOSTS_MULTIPLIER = 1.33,			-- Multiplier applied to the warscore value of major battle if the losing side is a claimant adventurer
	BATTLE_WARSCORE_LANDLESS_MULTIPLIER = 1.33,		-- Multiplier applied to the warscore value of major battle if the losing side has no holdings
	...
	REINFORCE_RATE = 0.015,							-- Reinforce rate per year
	...
	LEVY_RAISED_REINFORCE_RATE_MULTIPLIER = 0.1,	-- The rate at which the garrison/levy reinforce when the levy is raised
	...
	DAYS_UNTIL_HOLDER_GETS_WARSCORE = 730,			-- Days until the war score of the title owner starts increasing, if he controls the Holdings
	...
	ENEMY_SETTLEMENT_SUPPLY_BONUS = 0.0,			-- Supply bonus from enemy settlements(woot!)
	...
	ATTRITION_LEVEL_FACTOR = 0.001,					-- A general "attrition per month" multiplier
	ATTRITION_LEVEL_FACTOR_50_OVER = 0.001,			-- A general "attrition per month" multiplier when 50% over the supply limit
	ATTRITION_LEVEL_FACTOR_100_OVER = 0.001,			-- A general "attrition per month" multiplier when 100% over the supply limit
	...
	LOOT_TAX_MULTIPLIER = 0.5,						-- Base gold looted from raided holdings is (tax * LOOT_TAX_MULTIPLIER) + (total building value * LOOT_BUILDING_MULTIPLIER)
	...
	LOOT_HOLDING_DESTRUCTION_ODDS = 9999999,				-- a holding with less than LOOT_HOLDING_DEST_MIN_SAFE buildings will be destroyed by looting
	...
	RETINUE_FROM_REALMSIZE = 0.0,
	RETINUE_TITLE_MODIFIER = 0.0,
	RETINUE_INCREASE_PER_TECH = 0.0,
	...
	LIGHT_INFANTRY_MAINTENANCE = 0.5,
	...
	PIKEMEN_PHASE_MELEE_DEFENSE = 12,
	...
	KNIGHTS_MORALE = 15,
	KNIGHTS_MAINTENANCE = 2.5,
	KNIGHTS_PHASE_SKIRMISH_ATTACK = 1,
	KNIGHTS_PHASE_MELEE_ATTACK = 25,
	KNIGHTS_PHASE_PURSUE_ATTACK = 10,
	KNIGHTS_PHASE_SKIRMISH_DEFENSE = 10,
	KNIGHTS_PHASE_MELEE_DEFENSE = 15,
	KNIGHTS_PHASE_PURSUE_DEFENSE = 8,
	KNIGHTS_GRAPHICAL_FACTOR = 3,

	ARCHERS_MORALE = 1,
	ARCHERS_MAINTENANCE = 0.5,
	...
	SPECIAL_TROOPS_MORALE = 7,
	SPECIAL_TROOPS_MAINTENANCE = 2,
	SPECIAL_TROOPS_PHASE_SKIRMISH_ATTACK = 7,
	...
	MAX_RIVER_MOVEMENT_FORT_LEVEL = 20.0,
	...
	BOTTLENECK_CHOKE_TROOP_CAP = 600,		-- THIS IS SPARTAAAAAAAAA!!!!
	...
	OUTSIDE_LIEGE_LEVY_MULT = 0.75,				-- Multiplier to liege levies outside all capital regions
	
	SUPPLY_CAPACITY = 5000,						-- Amount of days of worth supply that a unit can hold.
	OUT_OF_SUPPLY_ATTRITION_BASE = 0.0001,		-- The attrition a unit that is out of supply takes.
	OUT_OF_SUPPLY_ATTRITION_INCREASE = 0.00001,	-- The increase per tick of attrition a unit that is out of supply takes.
	OUT_OF_SUPPLY_ATTRITION_TICK = 10,			-- The amount of days per tick for increase of attrition.
	...
	SHATTERED_RETREAT_PREFERRED_PROVINCES = 3,				-- Units will try to move at least this many provinces away in shattered retreat
	SHATTERED_RETREAT_MAX_PROVINCES = 5,					-- Units can not move longer than this many provinces during a shattered retreat
...
NTechnology = {
	DONT_EXECUTE_TECH_BEFORE = 7900,				-- Set this to your earliest starting year
	...
	NOMAD_TRIBUTARY_LEECH_RATIO = 0,			-- Nomads leeches 0.01% of the tech growth of their tributaries
...
NDisease = {
	...
	CROWDED_THRESHOLD_MODIFIER = 35, -- How many courtiers in a court to make it crowded and increase chance for disease.
...
NAI =
{
	...
	MIL_AI_COUNTER_INVADE_DISTANCE_THRESHOLD = 600,			-- Defenders in wars will not counter invade past this distance
	...
	MARRIAGE_AI_PRESTIGE_VALUE = 0.4,						-- Multiplier for how highly AI values prestige when arranging marriages and evaluating marriage offers
	...
	BASE_AI_ARMY_SIZE = 15000,								-- Base size that AI will use when forming armies (multiplied by supply tech)
	DESIRED_CONSORTS = 2,									-- AI will always want at least this many concubines, if they lack sons
	...
	RAID_MAX_REALM_SIZE = 9999,								-- Realms with more holdings than this will never go on Raids
	RAID_AGGRESSION = 50,									-- General frequency of raids. A LOWER number means more often!
	...
	FACTION_EXPIRATION_MONTHS = 48,							-- How long AI will cling onto weak factions
	FACTION_EXPIRATION_MONTHS_RANDOMNESS = 5,				-- Randomness to how long AI will cling onto weak factions
	FACTION_EXPIRATION_SIZE_LIEGE = 0.25,					-- How much smaller a faction must be compared to liege to be considered weak
	...
	FACTION_EXPIRED_MONTHS = 12,							-- How long AI will avoid factions after they have been expired
	...
	DIVINE_BLOOD_INCEST_FACTOR = 0.75,							-- Multiplier factor used for close relatives when finding best spouse in religion with divine blood enabled. Smaller factor leads to close relatives less likely to be a choice.
	...
	ADMINISTER_REALM_REALM_SIZE_THRESHOLD = 100,			-- If Empire and realm size is greater or equal to threshold then Steward will be assigned to Administer Realm job
...
NFrontend = 
{
	CAMERA_LOOKAT_X = 590.0, 			-- Rotation point in main menu
	CAMERA_LOOKAT_Y = 0.0,
	...
	CAMERA_START_X = 590.0,				-- Initial position in main menu
	CAMERA_START_Y = 150.0,				-- Y is height
	CAMERA_START_Z = 500.0,
	CAMERA_END_X = 590.0,				-- Move to position in main menu
	CAMERA_END_Y = 500.0,
	CAMERA_END_Z = 600.0,
